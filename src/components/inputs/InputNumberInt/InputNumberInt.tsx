...
import { InputNumberIntProps } from './types';
import { formatNumberIntValue, deformatNumberIntValue } from './utils';

const InputNumberInt: React.FC<InputNumberIntProps> = (props) => {
  const {
    id,
    size = 'm',
    error = false,
  ...
  const handleChange = (e: React.FormEvent<HTMLInputElement>) => {
    ...
    const fValue = formatNumberIntValue(pureValue, negative, formatted, maxLength);
    const dfValue = deformatNumberIntValue(fValue);
  ...
  const handleClear = () => {
    const fValue = formatNumberIntValue(null, negative, formatted, maxLength);
    const dfValue = deformatNumberIntValue(fValue);
    setPrevValue(dfValue);
    setFormattedValue(fValue);
  ...
  useEffect(() => {
    if (value !== prevValue) {
      setFormattedValue(formatNumberIntValue(value, negative, formatted, maxLength));
    }
  }, [value]);
  ...
  const formattedMaxLength = useMemo(
    ...
    [maxLength, formatted, negative, units],
  );
  ...
  return (
    <StyledInput sWidth={width}>
      <StyledInputBase
        ...
        inputMode={negative ? undefined : 'numeric'}
